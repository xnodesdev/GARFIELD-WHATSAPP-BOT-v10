  ...
  command.function(conn, mek, m,{from, l, quoted, body, isCmd, command, args, q, text, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, isCreator, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply})
    ...
    try {
        ...
        if (options.readViewOnce) {
            ...
            message.message = ephemeralMsg || message.message;
...
conn.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
    try {
        ...
        switch (type) {
            case "image":
                return conn.sendMessage(jid, { image: buffer, caption, ...options }, messageOptions);
            case "video":
                return conn.sendMessage(jid, {
                    video: buffer,
                    caption,
                    mimetype: subtype === "gif" ? undefined : 'video/mp4',
                    gifPlayback: subtype === "gif",
                    ...options
                }, messageOptions);
            case "audio":
                return conn.sendMessage(jid, { audio: buffer, caption, mimetype: 'audio/mpeg', ...options }, messageOptions);
            case "application":
                if (subtype === "pdf") {
                    return conn.sendMessage(jid, { document: buffer, mimetype: 'application/pdf', caption, ...options }, messageOptions);
                }
                break;
            default:
